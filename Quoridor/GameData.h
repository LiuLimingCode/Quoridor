...
public:
     ...
     */
    void gotoOrder(int player, Order order);
...
protected:
     ...
     */
    virtual bool isPlayer1Win(void) const { return CONDITION_PLAYER1_WIN(mPlayerPosition[0]) ? true : false; }
    virtual bool isPlayer2Win(void) const { return CONDITION_PLAYER2_WIN(mPlayerPosition[1]) ? true : false; }
    virtual bool isPlayer3Win(void) const { return CONDITION_PLAYER3_WIN(mPlayerPosition[2]) ? true : false; }
    virtual bool isPlayer4Win(void) const { return CONDITION_PLAYER4_WIN(mPlayerPosition[3]) ? true : false; }

    /*
     * 各个玩家的起始条件
     * @note    注意：该这些函数不允许用户调用,但是可以通过重载该函数修改游戏的起始条件
     */
    virtual void setPlayer1Start(void) { CONDITION_PLAYER1_START(mPlayerPosition[0]); }
    virtual void setPlayer2Start(void) { CONDITION_PLAYER2_START(mPlayerPosition[1]); }
